...
arena_cover = ['🛡Head', '🛡Body', '🛡Feet']
arena_attack = ['🗡Head', '🗡Body', '🗡Feet']
...
def parse_text(text, username, message_id):
    ...
    if bot_enabled and username == bot_username:
        ...
        if text.find('🌟Congratulations! New level!🌟') != -1 and lvl_up != 'lvl_off':
        ...
        elif 'You need to heal your wounds and recover, come back later.' in text:
            arena_delay = True
        ...
        elif 'In a dire need for an adventure, you went to a forest.' in text:
            log("Ушли в лес")
            lt_info = time()
            get_info_diff = random.randint(400, 500)
            endurance -= 1
        ...
        elif '📯Welcome to Arena!' in text:
            victory = re.search('Your rank: (\d+)', text).group(1)
            arenafight = re.search('Your fights: (\d+) / (\d+)', text)
            log('Поединков: {0} / {1}. Побед: {2}'.format(arenafight.group(1), arenafight.group(2), victory))
            if 'You need to heal your wounds and recover, come back later.' in text:
                arena_delay = True
                arena_delay_day = datetime.now(tz).day
                log("Отдыхаем денек от арены")
                arena_running = False
                sleep(random.randint(5, 15))
                action_list.append('⬅Back')
            ...
            if castle_name is None:
                log('Find casle name')
                log(text)
                log('flag:' + str(re.search('(.{2}).*, .+ Castle', text)))
            ...
            endurancetop = int(re.search('Stamina: (\d+)/(\d+)', text).group(2))
            gold = int(re.search('💰(-?[0-9]+)', text).group(1))
            inv = re.search('🎒Bag: ([0-9]+)/([0-9]+)', text)
            level = int(re.search('🏅Level: (\d+)', text).group(1))
            ...
            m = re.search('Battle of the seven castles in (?:(?:(\d+)ч)? ?(?:(\d+) minutes)?|a few seconds)', text)
            if not m and re.search('Межсезонье', text):
                m = re.search('Battle of the seven castles in (?:(?:(\d+)ч)? ?(?:(\d+) minutes)?|a few seconds)', 'Битва семи замков через 10000ч 100 минут')
            if not m.group(1):
                if m.group(2) and int(m.group(2)) <= 29:
                    report = True
                    state = re.search('State:\n(.*)', text).group(1)
                    if auto_def_enabled and time() - current_order['time'] > 1800 and 'Rest' in state:
                        if castle_name == 'red':
            ...
            if report:
                ...
                report = False
            if text.find('🛌Rest') == -1 and text.find('🛡Defend ') == -1:
                ...
                if text.find('🛌Rest') != -1 and arena_running:
                    arena_running = False
        ...
        elif arena_enabled and text.find('Choose point of attack and point of block.') != -1:
        ...
        elif text.find('Leaderborad of fighters') != -1 or text.find('Ничья') != -1:
            lt_info = time()
