...
def parse_text(text, username, message_id):
    ...
    if bot_enabled and username == bot_username:
        ...
        if text.find('🌟Congratulations! New level!🌟') != -1 and lvl_up != 'lvl_off':
        ...
        elif 'It’s hard to see your opponent in the dark. Wait until the morning.' in text:
            arena_delay = True
            ...
            inv = re.search('🎒Bag: ([0-9]+)/([0-9]+)', text)
            log('Рюкзак: {0} / {1}'.format(inv.group(1),inv.group(2)))
            if peshera_enabled and endurance >= 2 and text.find('🛌Rest') != -1:
                if les_enabled:
            ...
            elif les_enabled and not peshera_enabled and endurance >= 1 and orders['les'] not in action_list and text.find('🛌Rest') != -1:
                action_list.append(orders['quests'])
                action_list.append(orders['les'])

            elif arena_enabled and not arena_delay and gold >= 5 and not arena_running and text.find('🛌Rest') != -1:
        ...
        elif text.find('Leaderborad of fighters are updated:') != -1 or text.find('Draw') != -1:
        ...
        if bot_enabled and order_enabled and username in order_usernames:
            ...
            elif text.find(orders['deer']) != -1:
                update_order(orders['deer'])
            elif text.find(orders['wolf']) != -1:
                update_order(orders['wolf'])
            elif text.find(orders['moon']) != -1:
                update_order(orders['moon'])
            elif text.find('🌲') != -1:
